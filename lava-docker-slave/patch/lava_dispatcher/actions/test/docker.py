...
class DockerTestAction(Action, GetBoardId):
    ...
    def populate(self, parameters):
        ...
        if "role" in parameters:
            self.pipeline.add_action(DockerTestShellAction()(MultinodeTestAction)())
        else:
            self.pipeline.add_action(DockerTestShellAction()(TestShellAction)())
...
class DockerTestShellAction:
    def __call__(self, super_class):
        class DockerTestShell(super_class, GetBoardId, DeviceContainerMappingMixin):
            ...
            def run(self, connection, max_end_time):
                ...
                container = "lava-docker-test-shell-%s-%s" % (
                    self.job.job_id,
                    self.level,
                )
                ...
                if namespace:
                    downloads_dir = (
                        pathlib.Path(self.job.tmp_dir) / "downloads" / namespace
                    )
